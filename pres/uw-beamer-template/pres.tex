...
%	\multiinclude[<+->][format=jpg, graphics={ scale=0.25}]{gray_config}
        \begin{figure}
	  \includegraphics[scale=0.30]{orig_pos.jpg}
        \end{figure}
        \vspace{-0.5cm}
        \begin{figure}
	  \includegraphics[scale=0.30]{final_pos.jpg}
        \end{figure}
\column{0.55\textwidth}
\begin{figure}
%\centering
...
\centering
	...
	\resizebox{!}{5.7cm}{
        \begin{tikzpicture}[every text node part/.style={align=center}]
		% Optimized neutron transport
		\node [gscript] (adjnsrc) {Calculate time-integrated $q_n^+$};
...
\column{0.45\textwidth}
	\begin{itemize}
		...
		%\item{Map the structured (voxel) mesh to a tetrahedral mesh}
                \item{Map results onto mesh of geometry at each time step}
	\end{itemize}
\vspace{0.2cm}
%	\includegraphics[scale=0.29]{mesh_total.jpg}
	\includegraphics[scale=0.19]{adj_pflux_0_tet_box.jpg}
\vspace{0.1cm}
...
\column{0.45\textwidth}
	\begin{itemize}
		\item{Average over all time steps}
		\item{Map results onto mesh at original configuration}
	\end{itemize}
\vspace{0.2cm}
	\includegraphics[scale=0.16]{avg_adj_pflux_vox.jpg}
...
%	\caption{U.S. DOE Office of Fusion Energy Sciences (DE-FG02-99ER54513 and DE-SC0017122)}
\end{figure}
\begin{figure}
        \includegraphics[width=0.8\textwidth]{CHTC-logo.png}
%        \caption{UW-Madison Center for High Throughput Computing}
\end{figure}
\end{frame}
