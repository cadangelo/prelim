...
\begin{frame}{Time-integrated R2S: VR  for Neutron Transport }
	\begin{itemize}
...
\column{0.35\textwidth}
        \vspace{-3cm}
	\begin{itemize}
		\item{Perform adjoint photon transport at each time step 2}
	\end{itemize}
...
\begin{frame}{Experimental Steps}
	\begin{columns}
		\column{0.3\textwidth}
	\begin{enumerate}
	\item{Analog (No VR):}
...
\column{0.55\textwidth}
	...
	\resizebox{!}{3.9cm}{
		...
		photon transport\\ at each time step};
                \node [file, below of = pmc, yshift=0.91cm] (sdr) {Shutdown dose
		rate\\ at each time step};
		\path [line] (nmc) -- (nflux);
                \path [line] (nflux) -- (act);
                \path [line] (irrdec) -- (act);
