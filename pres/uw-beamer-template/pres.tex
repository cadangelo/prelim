...
\end{equation}
\item{$\sigma_{\overline{x}}$ is proportional
	to $1/\sqrt{\# histories}$} %\cite{mcnp_manual}.
...
\begin{frame}{VR for Multi-physics Analysis of Moving Systems}
	\begin{itemize}
		...
		\item{Movement during secondary step changes the construction
			of the adjoint neutron source}
	\end{itemize}
	\vspace{1cm}
	\centering
		{\textbf{Need to derive new adjoint neutron source}}
...
\column{0.5\textwidth}
	...
	\begin{itemize}
		\item{Generate geometry files at each time step of movement
			after shutdown}
	\multiinclude[<+->][format=jpg, graphics={ scale=0.3}]{gray_config}
	\end{itemize}
...
\centering
	...
	\resizebox{!}{11cm}{
		...
		\phantom{                \path [line] (pgeomi) -- (partgen);
...
%%%% 3. Map voxel to tet mesh and sum
\begin{frame}{TGT-CADIS: Generate VR Parameters}
\begin{columns}
\column{0.45\textwidth}
	\begin{itemize}
		\item{Map the voxel mesh to a tetrahedral mesh}
		\item{Average the adjoint photon flux calculated at each time
			step}
	\includegraphics[scale=0.30]{mesh_total.jpg}
...
\column{0.45\textwidth}
	\begin{itemize}
		...
		\item{Generate biased source and weight window mesh}
	\end{itemize}
...
\begin{frame}{Experiment: Toy Problem}
	...
	\item{No VR}
	\item{Photon VR: CADIS}
	\item{Neutron and Photon VR: TGT-CADIS}
\end{enumerate}
%\end{frame}
\column{0.55\textwidth}
	\begin{figure}
...
\begin{frame}{TODO}
	\begin{itemize}
		...
		\item{Add more on moving geom progress}
	\end{itemize}
\end{frame}
\end{document}
